...
[Bit sets][bitset] are a highly compact data structure that stores a list of bits. It is often used to represent a set of integers, or an array of booleans.

In addition to their compactness, bit sets enable extremely performant boolean operations like [unions][union] and [intersections][intersection], not to mention fast constant-time lookups.

In this series on bit sets, we'll walk through implementing a `BitSet` from scratch.
 ...
 * <a href="/blog/bit-sets-boolean-performance" target="_blank">Part 3</a> will tackle [boolean operations][boolean_operations] and go in-depth on performance.
...
[boolean_operations]: https://en.wikipedia.org/wiki/Boolean_algebra#Boolean_operations
...
Bit masks allows us compactly store multiple booleans in a single number. You may know bit masks under the names _bit flags_, or _bit field_.
...
We can set bits to 0 through the use of the bitwise NOT operator `~`, a [unary operator][unary_operator] that flips all of the bits in the number:
...
This is where bit sets come in. Once you need to store more than 32 booleans (or 64, depending on the language), you can use an array of integers to make the possible number of bits _arbitrarily_ large.
...
Perhaps more importantly, set operations (like union, intersection, subtraction) are stupid fast. We'll explore this in depth in <a href="/blog/bit-sets-boolean-performance" target="_blank">part 3</a>.
...
So the skeleton for our `BitSet` will look like so:
...
The `add` method should set the bit at `index` to 1. This bit will live in some individual `word`, which we can find via `wordIndex`:
...
We can set a bit at a specific index in a number to 1 like so:
...
`<<` is the bitwise left shift operator, which returns the input number with its bits shifted left by N places.
...
If the result is non-zero, the bit is set to 1. And that's our implementation of `BitSet.has`.
