...
<SmallNote moveCloserUpBy={16}>`Equals` as used above asserts that the two arguments are equal.</SmallNote>
...
<SmallNote moveCloserUpBy={16}>This recursive solution is terrible for memory usage. We're constructing a new array in every iteration. An iterative approach with a while loop would be optimal.</SmallNote>
