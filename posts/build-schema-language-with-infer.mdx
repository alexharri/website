...
The `infer` keyword in TypeScript, especially when combined with recursive types, is incredibly powerful. It enables you to transform types in complex and intricate ways that, honestly, you shouldn't be able to.

In this article, we'll solve the problems I encountered in building a [real schema builder library](https://github.com/alexharri/strema) that makes extensive use of `infer` and recursive types.
...
This is all fine, but I would love to be able to use a more terse syntax to describe the schema.

To start, I want to describe the optionality of a field in the same way as TypeScript does for interfaces:
...
If we were to accept a list of values, I would like to use an array literal syntax like so:
...
The variables `K` and `V` variables are **only** created if `T` matches the pattern.
...
The first big hurdle we encounter are objects with multiple properties.
...
This is our first example of recursive types.
...
I hope I was successful in showing how powerful `infer` and recursive types are in TypeScript.
...
[source-code]:https://github.com/alexharri/website/blob/e5784d727348dfe4e597fc3973ca44c9e199b2cd/code-samples/Parse.ts

If you would like to take a look at the open source library I built, check out [`strema` on GitHub](https://github.com/alexharri/strema). It's more mature and implements the following:
