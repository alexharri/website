...
Everyone knows the basics. Use the arrow keys to move the cursor, and hold shift to select text when moving with the arrow keys.
...
You can jump over words with the arrow keys by holding down `Option`.
...
<CodeScript id="jump-over-words" script={[
  { command: "Select", line: 1, col: 1 },
  { command: "Option Right", times: 5, msBetween: 300 },
  { command: "Option Left", times: 5, msBetween: 300 },
]} />

Additionally, holding down `Shift` allows you to shift your selection by words.

```
// @script jump-over-words-select
one two three four five.
```

<CodeScript id="jump-over-words-select" script={[
  { command: "Select", line: 1, col: 1 },
  { command: "Shift Option Right", times: 5, msBetween: 300 },
  { command: "Shift Option Left", times: 5, msBetween: 300 },
]} />

To jump to the end or beginning of a line, you can combine `Command` with the arrow keys.
...
This is useful, for example, when selecting blocks of code:
...
Let's apply the text navigation knowledge we just learned with a concrete example.
