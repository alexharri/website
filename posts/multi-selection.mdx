...
Early in my career, I discovered a trick that has made me very productive when writing code. The trick does not involve learning Vim, and it does not involve any magic plugin.

I've used this trick with Sublime Text, Atom, and now VS Code. The trick is basic enough that every text editor I've used supports it out of the box.

The trick is broadly applicable. I use it with multiple programming languages, and I use it when performing non-programming text editing tasks such as formatting, filtering, or selecting data.
...
Before diving into the trick, let's understand the problem that it solves.
...
Matching the `if` if somewhat obvious, especially since each of the `if` statements have an identical structure.

Though the pattern I want to match are typically uniform, sometimes I encounter more complex patterns. These can come in the form of
...
In making testing the library less verbose, we developed a test utility function that takes an expression, and its expected value.

```tsx
function expectEqual(expression: string, expectedValue: any): void;
```

We now have test code that looks like so:
...
Since we have a lot of tests, doing this manually would be a lot of work. Luckily for us, we know how to use `Command D`. We just need to find a pattern to match.
...
This one's definitely not as obvious as our `if` to `switch` example.
...
For this reason, I prefer `Command D` when working with a small number of matches. The matching feels more local as you visually see every match happen.
...
To me, this is the least intuitive part of using `Command D`. It eventually becomes second-nature, given enough practice.
...
There is no pattern across these lines, so matching each lime seems hard. However, `Command D` allows us to match line breaks.
...
Translating between cases (such as changing snake-case to camelCase) comes up from time-to-time. I often encounter this case when working across HTML, CSS and JavaScript.
