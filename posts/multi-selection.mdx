...
Skipping a match comes up now and then. To skip an element _once you've matched it_, press `Command K` followed by `Command D`.
...
There is not a direct keyboard shortcut for the `Transform to Uppercase` command. Run it by opening the command prompt with `Shift Command P` and then typing the name of the command.

<SmallNote>There's also a `Transform to Lowercase` command.</SmallNote>


## Use cases for `Command D`

Now that we've covered every major feature of `Command D`, I want to run through a few examples where `Command D` is useful.

These may help give you an idea of where `Command D` can be used.
...
```javascript
// @script copy-names expectedLines=9
const mul = (a, b) => a * b;
const div = (a, b) => a / b;
const exp = (a, b) => a ** b;
```

<CodeScript id="copy-names" script={[
  { command: "Shift Option Right" },
  { command: "Command D", times: 2 },
  { command: "Right", times: 2 },
  { command: "Shift Option Right" },
  { command: "Copy" },
  { command: "Command Down" },
  { command: "Command Enter", times: 2 },
  { command: "Type", text: "export const math = " },
  { command: "Wrap", text: "{}" },
...
Here we take a list of users (name and email) and format them as email strings.
...
## Move properties to a single line
