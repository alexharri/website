  ...
  private static selectUsingExtremes(
    ...
    for (let dim = 0; dim < dimensions && selected.length < maxCount; dim++) {
      ...
      if (minIdx !== -1 && !used.has(minIdx) && selected.length < maxCount) {
        ...
        if (!isSimilar || selected.length === 0) {
          ...
          console.log(
            `  Skipped similar min dim ${dim}: '${chars[minIdx]}' (${minVal.toFixed(3)})`,
          );
        }
      }
      ...
      if (maxIdx !== -1 && !used.has(maxIdx) && selected.length < maxCount) {
        ...
        if (!isSimilar || selected.length === 0) {
          ...
          console.log(
            `  Skipped similar max dim ${dim}: '${chars[maxIdx]}' (${maxVal.toFixed(3)})`,
          );
        }
      }
    }
    ...
    for (const candidate of candidates) {
      ...
      if (!isSimilar) {
        ...
        console.log(
          `  High variance: '${chars[candidate.index]}' (variance: ${candidate.variance.toFixed(
            3,
          )})`,
        );
      } else {
        console.log(
          `  Skipped similar variance: '${
            chars[candidate.index]
          }' (variance: ${candidate.variance.toFixed(3)})`,
        );
      }
    }
