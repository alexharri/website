  ...
  async saveCharacterVectors(
    ...
    for (let row = 0; row < this.samplingConfig.gridRows; row++) {
      for (let col = 0; col < this.samplingConfig.gridCols; col++) {
        ...
        gridCells.push({ row, col, externalOffsets });
      }
    }
...
async function generateAlphabetManager() {
  ...
  const imports = configNames
    .map((name) => `import ${name.replace(/-/g, "")}Alphabet from "./${name}.json";`)
    .join("\n");

  // Generate alphabets object
  const alphabetEntries = configNames
    .map((name) => `  "${name}": ${name.replace(/-/g, "")}Alphabet,`)
    .join("\n");
