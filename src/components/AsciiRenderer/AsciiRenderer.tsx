...
export function AsciiRenderer(props: Props) {
  ...
  const [spacingMetadata, setSpacingMetadata] = useState<{ letterSpacing: string; lineHeight: number } | null>(null);
  const [currentMetadata, setCurrentMetadata] = useState<{ width: number; height: number } | null>(null);
      ...
      {visualizationData && (
        <div className={s("visualizationLayer")}>
          {visualizationData.samplingPoints.map((point, index) => {
            const shouldShow = point.isExternal ? props.showExternalPoints : props.showSamplingPoints;
